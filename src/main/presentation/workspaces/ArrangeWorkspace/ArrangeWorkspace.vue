    ...
    <div class="arrange-workspace__tracks">
      <article v-for="track in store.project.tracks" :key="track.id" :class="{ 'is-muted': track.muted }">
        <div class="arrange-workspace__track-controls">
          ...
          <label class="arrange-workspace__volume"><span class="sr-only">Volume for {{ track.name }}</span><input :value="track.volume" type="range" min="0" max="1" step="0.01" @input="store.updateTrackVolume(track.id, Number(($event.target as HTMLInputElement).value))" /></label>
