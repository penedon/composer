...
test('semantic wheel exposes shades, blends, weights, and unavailable emotions', async ({ page }, testInfo) => {
  ...
  await expect(page.locator('.emotion-picker__details')).toHaveCount(0)
  const center = map.locator('.emotion-semantic-wheel__center')
  await expect(center).toContainText('Longing')
  await expect(center).toContainText('Emotional makeup')
  await expect(center).toContainText('Love70%')
  await expect(center).toContainText('Sadness60%')
  await expect(center).toContainText('Desire50%')
