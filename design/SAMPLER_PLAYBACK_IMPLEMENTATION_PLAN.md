...
| BP-07 | Send sampler output directly to `context.destination` | Track volume and future mixing are bypassed | Route through per-track gains; graph/destination test | Fixed |
| BP-08 | Apply volume and velocity in multiple gain stages without calibration | Quiet tracks disappear or loud presets clip | Separate voice velocity, track gain, and preset compensation; peak/manual comparison check | Fixed in audio graph; final listening calibration remains open |
